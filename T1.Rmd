...
randomAttr <- function(data){
  colId <- sample(1:ncol(data), sqrt(ncol(data)), replace=FALSE)
  print(colId)
  return(colId)
}
```
...
bootstraping <- function(data, indexClass){
  ...
  result <- vector()
  acc <- 0
  for(i in 1:nrow(testDataL)){
    result[i] <- walkTree(tree, testData = testDataL[i,])
    print(result[i])

    if(testDataL[i,indexClass] == result[i]){
      acc <- acc+1
      
    }
  }
  return(acc/nrow(testDataL))
}
```
...
results <- bootstraping(data, indexClass)

# tree <- Node$new("ROOT")
# tree <- computeTree(tree, data, indexClass)
# tree
# 
# testData <- read.csv("testePercorre.csv",header = TRUE,stringsAsFactors = FALSE,sep=";")
# foo <- walkTree(tree)
...
# tree2
# 
# testData <- read.csv("trainTest.csv", header = TRUE, stringsAsFactors = FALSE, sep = "\t")
# testData$target <- as.logical(testData$target)
# result2 <- walkTree(tree2)
```