...
for(i in 1:(nrow(test))){
  tree$AddChild(test[[1]][i])
  subData[[i]] <- data[which(data[[maxGain]]==test[[1]][i]),]
  if(length(unique(subData[[i]][,indexClass])) > 1){
  entropyClass <- computeEntropy(subData[[i]], NULL, indexClass)
  for(d in 1:(ncol(subData[[i]])-1)){
    if(d != maxGain){
    entropies[d] <- entropyClass - computeEntropy(subData[[i]], attIndex = d, indexClass)
      } else entropies[d] = -99
  }
  maxGain2 <- which.max(entropies)
  print(subData[[i]])
  test <- unique(subData[[i]][maxGain2])
   ## A partir daqui é recursivo
  for(j in 1:(nrow(test))){
        subData2[[j]] <- subData[[i]][which(subData[[i]][[maxGain2]]==test[[1]][j]),]
  }
}
}
